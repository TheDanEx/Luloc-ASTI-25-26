...
static esp_eth_handle_t eth_init_internal(esp_eth_mac_t **mac_out, esp_eth_phy_t **phy_out)
{
    ...
    // Init vendor specific MAC config to default, and create new ESP32 Ethernet MAC instance
...
/**
 * @brief High-level Ethernet initialization
 * 
 * Initializes the complete Ethernet stack including TCP/IP, event loop,
 * network interfaces, and starts the Ethernet driver.
 */
esp_err_t ethernet_init(void)
{
    ...
    ret = example_eth_init(&s_eth_handles, &s_eth_cnt);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "example_eth_init failed");
        return ret;
    }
    ...
    // Allocate memory for network interfaces and glues
    s_eth_netifs = calloc(s_eth_cnt, sizeof(esp_netif_t *));
    if (s_eth_netifs == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for eth_netifs");
        ret = ESP_ERR_NO_MEM;
        goto cleanup_event_loop;
    }

    s_eth_netif_glues = calloc(s_eth_cnt, sizeof(esp_eth_netif_glue_handle_t));
    if (s_eth_netif_glues == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for eth_netif_glues");
        ret = ESP_ERR_NO_MEM;
        goto cleanup_netifs_mem;
    }
    ...
    if (s_eth_cnt == 1) {
        ...
        s_eth_netifs[0] = esp_netif_new(&cfg);
        if (s_eth_netifs[0] == NULL) {
            ESP_LOGE(TAG, "Failed to create esp_netif");
            ret = ESP_FAIL;
            goto cleanup_glues_mem;
        }

        s_eth_netif_glues[0] = esp_eth_new_netif_glue(s_eth_handles[0]);
        if (s_eth_netif_glues[0] == NULL) {
            ESP_LOGE(TAG, "Failed to create eth_netif_glue");
            ret = ESP_FAIL;
            goto cleanup_glues_mem;
        }

        // Attach Ethernet driver to TCP/IP stack
        ret = esp_netif_attach(s_eth_netifs[0], s_eth_netif_glues[0]);
        if (ret != ESP_OK) {
        ...
        for (int i = 0; i < s_eth_cnt; i++) {
            ...
            if (s_eth_netifs[i] == NULL) {
                ESP_LOGE(TAG, "Failed to create esp_netif at index %d", i);
                ret = ESP_FAIL;
                goto cleanup_glues_mem;
            }

            s_eth_netif_glues[i] = esp_eth_new_netif_glue(s_eth_handles[i]);
            if (s_eth_netif_glues[i] == NULL) {
                ESP_LOGE(TAG, "Failed to create eth_netif_glue at index %d", i);
                ret = ESP_FAIL;
                goto cleanup_glues_mem;
            }

            // Attach Ethernet driver to TCP/IP stack
            ret = esp_netif_attach(s_eth_netifs[i], s_eth_netif_glues[i]);
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "esp_netif_attach failed at index %d", i);
                goto cleanup_glues_mem;
            }
        }
    }
    ...
    // Parse IP addresses from Kconfig strings
    esp_ip4_addr_t ip_addr, netmask, gateway;
    ESP_ERROR_CHECK(esp_netif_str_to_ip4(CONFIG_ROBOT_ETH_STATIC_IP_ADDR, &ip_addr));
    ESP_ERROR_CHECK(esp_netif_str_to_ip4(CONFIG_ROBOT_ETH_STATIC_NETMASK, &netmask));
    ESP_ERROR_CHECK(esp_netif_str_to_ip4(CONFIG_ROBOT_ETH_STATIC_GATEWAY, &gateway));
    ...
    ret = esp_netif_set_ip_info(s_eth_netifs[0], &ip_info);
    if (ret != ESP_OK) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "eth_event_handler_register failed");
        goto cleanup_glues_mem;
    }

    ret = esp_event_handler_register(IP_EVENT, IP_EVENT_ETH_GOT_IP, &got_ip_event_handler, NULL);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "got_ip_event_handler_register failed");
...
cleanup_glues_mem:
    free(s_eth_netif_glues);
    s_eth_netif_glues = NULL;
cleanup_netifs_mem:
    free(s_eth_netifs);
    s_eth_netifs = NULL;
cleanup_event_loop:
...
cleanup_netif:
    esp_netif_deinit();
cleanup_eth:
    example_eth_deinit(s_eth_handles, s_eth_cnt);
    return ret;
}

/**
 * @brief High-level Ethernet de-initialization
 */
esp_err_t ethernet_deinit(void)
{
    esp_err_t ret = ESP_OK;

    if (s_eth_handles == NULL || s_eth_cnt == 0) {
    ...
    for (int i = 0; i < s_eth_cnt; i++) {
        if (s_eth_handles[i] != NULL) {
            ret = esp_eth_stop(s_eth_handles[i]);
            if (ret != ESP_OK) {
                ESP_LOGW(TAG, "esp_eth_stop failed at index %d", i);
            }
        }
    }

    // Destroy network interfaces and glues
    for (int i = 0; i < s_eth_cnt; i++) {
        if (s_eth_netif_glues[i] != NULL) {
            esp_eth_del_netif_glue(s_eth_netif_glues[i]);
        }
        if (s_eth_netifs[i] != NULL) {
            esp_netif_destroy(s_eth_netifs[i]);
        }
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "esp_event_loop_delete_default failed");
    }

    // Deinitialize Ethernet driver
    ret = example_eth_deinit(s_eth_handles, s_eth_cnt);
    s_eth_handles = NULL;
    s_eth_cnt = 0;
