...
// Read a single sensor multiple times and yield an average (High precision)
...
// Background Task for Continuous High-Frequency Calibration
    ...
    while(ctx->is_calibrating) {
        ...
        for (int i = 0; i < ctx->config.num_sensors; i++) {
            ...
            if (read_sensor_averaged(ctx, i, &val) == ESP_OK) {
                ...
                // Check if this particular sensor reached the threshold
        ...
        ctx->is_fully_calibrated = all_calibrated_so_far; // Flips to true only when EVERY sensor sees contrast
        
        xSemaphoreGive(ctx->mutex);
        
        // Yield to avoid starving lower priority tasks on Core 1, but sample very fast (5ms = 200Hz)
...
line_sensor_handle_t line_sensor_init(const line_sensor_config_t *config)
{
    ...
    // Apply defaults from Kconfig if set to 0
    ...
    // Initialize calibration targets: min expects max possible ADC (4095), max expects 0.
    ...
    // Initialize Modern ESP-IDF ADC Oneshot API
    ...
    ESP_LOGI(TAG, "Line Sensor Array initialized [%d sensors | Threshold: %d | Avg count: %d]", 
             config->num_sensors, ctx->config.calibration_threshold, ctx->config.oversample_count);
...
esp_err_t line_sensor_calibration_start(line_sensor_handle_t handle)
{
    ...
    if (ctx->is_calibrating) return ESP_OK; // Already running
    ...
    // Spawn a FreeRTOS task to sample at maximum speed in background on Core 1
...
esp_err_t line_sensor_calibration_stop(line_sensor_handle_t handle)
{
    ...
    ctx->is_calibrating = false; // Flag tells task to exit cleanly
...
esp_err_t line_sensor_read(line_sensor_handle_t handle, line_sensor_data_t *out_data)
{
    ...
    for (int i = 0; i < ctx->config.num_sensors; i++) {
        ...
        // Physical Interpolation (Noise Gate: ignores signals less than 5% reflection)
        if (weight > 0.05f) { 
            sum_weights += weight;
