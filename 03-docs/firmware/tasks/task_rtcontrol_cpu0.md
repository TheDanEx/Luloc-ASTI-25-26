...
- **Apropiación del Núcleo Crítica (Core Hoarding):** Jamás este bucle debe bloquear al procesador (Ej: Olvidar el `vTaskDelay` interno); si el `while(1)` corre libre forzará al "Idle Task" a fallar para el núcleo 0, provocando un crasheo del System Task Watchdog (`Task watchdog got triggered`) reiniciando toda la placa en milisegundos.
